...
Prepared as Phase 6's "one usability pass" line item (`docs/ui_ux_elevation_plan.md`). This needs real human participants and a moderator, so it isn't something that can be executed autonomously — this document is the script for whoever on the team runs it. Budget ~30 min per session.
