...
test('a client can reset their password and log in with the new one', async ({ page, cleanup }) => {
  const user = await signUpFlexPro(page, 'client', cleanup)
  await confirmTestUserEmail(user.email, 'client')
