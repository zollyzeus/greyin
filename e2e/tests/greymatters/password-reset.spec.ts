...
import { signUpGreyMatters } from '../../utils/auth'
import { confirmTestUserEmail, getRecoveryOtp } from '../../utils/admin'
import { waitForURLResilient } from '../../utils/nav'

test('a reader can reset their password and log in with the new one', async ({ page, cleanup }) => {
  const user = await signUpGreyMatters(page, cleanup)
  await confirmTestUserEmail(user.email, 'author')
