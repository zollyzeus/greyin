...
import { Telescope, ArrowLeft, Bell } from 'lucide-react'
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href="/dashboard" className="flex items-center text-gray-600 hover:text-amber-700 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-md divide-y">
          {notifications && notifications.length > 0 ? (
            notifications.map((n) => {
              ...
              return external ? (
                <a key={n.id} href={href} className="block p-4 hover:bg-gray-50">
                  {content}
                </a>
              ) : (
                <Link key={n.id} href={href} className="block p-4 hover:bg-gray-50">
                  {content}
                </Link>
              )
            })
