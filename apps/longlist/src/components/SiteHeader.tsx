    ...
    <header className="bg-white border-b">
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        <a href="https://greyin.net" className="flex items-center min-w-0">
          <Telescope className="h-8 w-8 text-amber-700 flex-shrink-0" />
          <span className="ml-2 text-2xl font-bold truncate">Longlist</span>
        </a>

        <nav className="hidden md:flex items-center gap-6 flex-shrink-0">
          <a href="https://greyin.net" className="flex items-center gap-1 text-gray-700 hover:text-amber-700" title="Back to the Greyin ecosystem hub">
            <Home className="h-4 w-4" />
            Home
          </a>
          <div className="relative group">
            <button className="text-gray-700 hover:text-amber-700 transition flex items-center gap-1">
              Explore
            ...
            <div className="absolute left-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all duration-200">
              {NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href} className="block px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-amber-700 transition">
                  {link.label}
                </Link>
              ))}
              <a href="https://greyin.net/feedback?app=longlist" className="flex items-center gap-2 px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-amber-700 transition border-t border-gray-100" title="Send feedback to the Greyin team">
          ...
          {isLoggedIn === true ? (
            <>
                <NotificationBell />
              <Link href="/dashboard" className="text-gray-700 hover:text-amber-700">Dashboard</Link>
            ...
            <>
              <Link href="/login" className="text-gray-700 hover:text-amber-700">Sign In</Link>
      ...
      {mobileOpen && (
        <nav className="md:hidden border-t bg-white px-4 py-4 space-y-1">
          <a href="https://greyin.net" className="flex items-center gap-2 py-2.5 text-gray-700 hover:text-amber-700" onClick={() => setMobileOpen(false)}>
            <Home className="h-4 w-4" />
            Home
          </a>
          ...
          {exploreOpen && (
            <div className="pl-3 space-y-1 pb-1">
              {NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href} className="block py-2 text-sm text-gray-600 hover:text-amber-700" onClick={() => setMobileOpen(false)}>
                  {link.label}
                </Link>
              ))}
              <a href="https://greyin.net/feedback?app=longlist" className="flex items-center gap-2 py-2 text-sm text-gray-600 hover:text-amber-700" onClick={() => setMobileOpen(false)}>
                <MessageSquareHeart className="h-4 w-4" />
                Feedback
              </a>
            </div>
