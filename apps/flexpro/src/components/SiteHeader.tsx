...
import { Briefcase, Menu, X, ChevronDown, Home, MessageSquareHeart, Building2, BookOpen, Users, FlaskConical, Telescope } from 'lucide-react'
...
const NAV_LINKS = [
  ...
  { href: '/gigs/new', label: 'Become a Seller' },
...
/**
 * The homepage previously hand-rolled a bare-bones nav with no signup
 * CTA at all (only "Browse Gigs" / "Sign In") while every inner page had
 * a full nav with a "Join Now" button -- and neither had any mobile
 * behavior. One shared header fixes both.
 */
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <a href="https://greyin.net" className="flex items-center min-w-0">
            <Briefcase className="h-8 w-8 text-orange-600 dark:text-orange-500 flex-shrink-0" />
            <span className="ml-2 text-2xl font-bold text-gray-900 dark:text-gray-50 truncate">FlexPro</span>
          </a>
          ...
          <div className="pt-3 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-2">
            {isLoggedIn === true ? (
              <>
                ...
                <form action="/auth/logout" method="POST">
                  <button type="submit" className="w-full text-center py-2.5 bg-orange-600 text-white rounded-lg">
                    Sign Out
                  </button>
                </form>
              </>
