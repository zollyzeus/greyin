...
export async function POST(request: Request) {
  ...
  // Records that this account is active on the FlexPro pillar — a no-op
  // if it already knows that, but lets someone who signed up on a
  // *different* pillar and is now logging into FlexPro for the first
  // time (shared credentials via SSO) show up in the cross-pillar view.
  await supabase.rpc('ensure_pillar_membership', { p_pillar: 'flexpro', p_default_role: 'client' })
