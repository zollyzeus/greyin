  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href="/client-jobs" className="flex items-center gap-2 text-gray-600 hover:text-orange-600 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-8 mb-6">
          <div className="flex items-start justify-between">
            ...
            {job.budget_amount && <span className="text-xl font-bold text-gray-900">₹{job.budget_amount.toLocaleString('en-IN')}</span>}
          </div>
        ...
        {isOwner && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Applicants ({(applications || []).length})</h2>
            {applications && applications.length > 0 ? (
              <div className="space-y-4">
                {applications.map((app: any) => (
                  <div key={app.id} className="border border-gray-200 rounded-lg p-4 flex items-start justify-between">
                    <div>
                      <p className="font-medium text-gray-900">{app.freelancer?.full_name || 'A freelancer'}</p>
                      {app.cover_note && <p className="text-sm text-gray-600 mt-1">{app.cover_note}</p>}
                      <p className="text-sm font-semibold text-gray-900 mt-2">₹{app.proposed_price.toLocaleString('en-IN')}</p>
                      <span className={`inline-block mt-1 text-xs font-medium capitalize ${
                        app.status === 'pending' ? 'text-gray-500' : app.status === 'accepted' ? 'text-green-600' : 'text-red-500'
                      }`}>
                        {app.status}
                      </span>
                    </div>
        ...
        {!isOwner && job.status === 'open' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            {myApplication ? (
              ...
              <>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Apply — free, no subscription needed</h2>
                <form action={`/api/client-jobs/${job.id}/apply`} method="POST" className="space-y-4">
                  <div>
                    <label htmlFor="proposed_price" className="block text-sm font-medium text-gray-700">Your proposed price (₹)</label>
                  ...
                  <div>
                    <label htmlFor="cover_note" className="block text-sm font-medium text-gray-700">Cover note (optional)</label>
              ...
              <Link href={`/login?next=/client-jobs/${job.id}`} className="text-orange-600 font-medium hover:text-orange-700">
                Sign in to apply
              </Link>
            )}
          </div>
