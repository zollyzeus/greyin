...
export async function POST(request: Request) {
  ...
  // Soft assignment, not a rejection -- signup stays open for everyone as
  // a follower. The real gate is the RLS policy on posts (migration 038),
  // which checks is_verified_expert directly and so also recognizes
  // someone who earns authorship later via Greyin Score without a stored
  // role update.
  const { error: authError } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        ...
        last_name: lastName,
        role: yearsExperience != null && yearsExperience >= 12 ? 'author' : 'follower',
