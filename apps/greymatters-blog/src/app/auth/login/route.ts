...
export async function POST(request: Request) {
  ...
  // Records that this account is active on the GreyMatters pillar — a
  // no-op if it already knows that, but lets someone who signed up on a
  // *different* pillar and is now logging into GreyMatters for the first
  // time (shared credentials via SSO) show up in the cross-pillar view.
  await supabase.rpc('ensure_pillar_membership', { p_pillar: 'greymatters', p_default_role: 'follower' })
