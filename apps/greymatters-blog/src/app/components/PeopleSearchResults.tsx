...
export function PeopleSearchResults({ results }: { results: PersonResult[] }) {
  ...
  return (
    <div className="mt-8">
      <h2 className="text-lg font-semibold text-gray-900 mb-4 dark:text-gray-50">People on Greyin</h2>
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 bg-teal-100 rounded-full flex items-center justify-center flex-shrink-0 dark:bg-teal-950/40">
                  <User className="h-5 w-5 text-teal-600 dark:text-teal-400" />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-medium text-gray-900 dark:text-gray-50">{p.full_name || 'Greyin member'}</h3>
                    {p.is_verified_expert && <BadgeCheck className="h-4 w-4 text-teal-600 flex-shrink-0 dark:text-teal-400" />}
                  </div>
