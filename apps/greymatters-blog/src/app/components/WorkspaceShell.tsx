...
// (home base) > My Posts (checking status/stats, frequent) > Write New
// Post (high-impact but occasional, one per article).
const BASE_NAV = [
    ...
    <>
      <div className={`flex items-center h-16 border-b border-gray-200 dark:border-gray-800 shrink-0 ${collapsed ? 'justify-center px-2' : 'gap-2 px-5'}`}>
        <a href="https://greyin.net" className="flex items-center gap-2">
          <BookOpen className="h-6 w-6 text-sky-600 dark:text-sky-400 shrink-0" />
          {!collapsed && <span className="text-lg font-bold text-gray-900 dark:text-gray-50">GreyMatters</span>}
        </a>
      </div>
      ...
      <div className="border-t border-gray-200 p-3 space-y-2 shrink-0 dark:border-gray-800">
        ...
        {greyinScore != null && collapsed && (
          <div
            className="flex items-center justify-center px-2 py-2 rounded-lg bg-sky-50 dark:bg-sky-950/40"
            data-testid="dashboard-score-badge"
            title={`Greyin Score ${greyinScore}`}
          >
            <span className="text-xs font-bold text-sky-700 dark:text-sky-400">{greyinScore}</span>
          </div>
        )}
        <div className={`flex items-center px-3 py-1.5 ${collapsed ? 'justify-center px-2' : 'gap-2'}`}>
          <div
            className="h-8 w-8 rounded-full bg-sky-100 text-sky-700 dark:bg-sky-950 dark:text-sky-400 flex items-center justify-center text-xs font-semibold shrink-0"
