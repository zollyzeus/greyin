...
import { Briefcase, ArrowLeft, TrendingUp } from 'lucide-react'
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Salary Trends</h1>
        </div>
        ...
        <form className="bg-white rounded-lg shadow-md p-6 mb-8 grid grid-cols-1 sm:grid-cols-4 gap-3">
          <input
            type="text"
            name="role"
            defaultValue={role || ''}
          ...
          />
          <select name="level" defaultValue={level || ''} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            <option value="">Any level</option>
            {LEVELS.map((l) => (
              <option key={l} value={l}>{l.charAt(0).toUpperCase() + l.slice(1)}</option>
            ))}
          </select>
          <select name="experience_bucket" defaultValue={experienceBucket || ''} className="border border-gray-300 rounded-lg px-3 py-2 text-sm">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          {role ? (
            <>
              ...
              <form action="/api/salary-trends/watch" method="POST" className="mt-4 pt-4 border-t border-gray-100">
                <input type="hidden" name="role" value={role} />
                {level && <input type="hidden" name="level" value={level} />}
                {location && <input type="hidden" name="location" value={location} />}
                {isWatching ? (
                  <p className="text-sm text-gray-500">You&rsquo;re watching this trend &mdash; we&rsquo;ll notify you of a material move.</p>
                ) : (
                  <button type="submit" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700">
                    Watch this trend
                  </button>
                )}
              </form>
            ...
            <p className="text-gray-500 text-sm">Enter a role above to see salary trends.</p>
          )}
        </div>
      </div>
    </main>
