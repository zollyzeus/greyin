...
function RailContents({ variant, role, activeSection, userName, verified, greyinScore, collapsed, onNavigate, onOpenFeedback, onLogNav, onLogPillarSwitch }: {
  variant: 'candidate' | 'employer'
  role?: string | null
  activeSection?: string
  userName: string
    ...
    <>
      <div className={`flex items-center h-16 border-b border-gray-200 dark:border-gray-800 shrink-0 ${collapsed ? 'justify-center px-2' : 'gap-2 px-5'}`}>
        <a href="https://greyin.net" className="flex items-center gap-2" title="Go to Greyin Hub">
          ...
          {!collapsed && <span className="text-lg font-bold text-gray-900 dark:text-gray-50">DeepEdge</span>}
        </a>
      </div>
      ...
      <div className="border-t border-gray-200 p-3 space-y-2 shrink-0 dark:border-gray-800">
        ...
        {greyinScore != null && collapsed && (
          <div
            className="flex items-center justify-center px-2 py-2 rounded-lg bg-indigo-50 dark:bg-indigo-950/40"
            data-testid="dashboard-score-badge"
            title={`Greyin Score ${greyinScore}`}
          >
            <span className="text-xs font-bold text-indigo-700 dark:text-indigo-400">{greyinScore}</span>
          </div>
        )}
        <div className={`flex items-center px-3 py-1.5 ${collapsed ? 'justify-center px-2' : 'gap-2'}`}>
          <div
            className="h-8 w-8 rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-400 flex items-center justify-center text-xs font-semibold shrink-0"
        ...
        <form action="/auth/logout" method="post">
          <button
            ...
            className={`w-full flex items-center rounded-lg text-sm text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-800 ${collapsed ? 'justify-center px-2 py-2' : 'gap-2 px-3 py-2'}`}
          >
            <LogOut className="h-4 w-4 shrink-0" aria-hidden="true" /> {!collapsed && 'Sign out'}
          </button>
        </form>
      </div>
    </>
...
export function WorkspaceShell({ activeSection, variant = 'candidate', role, userName, verified, greyinScore, pageTitle, children }: WorkspaceShellProps) {
  ...
  // Collapsible rail (2026-09-13): persisted per-browser, not per-user --
  // matches the theme toggle's own localStorage-only persistence rather
  // than a profiles column, since this is a pure display preference with
  // no cross-device value. Defaults to expanded (false) so there's no
  // layout shift/flash before this reads localStorage on mount.
  ...
  useEffect(() => {
    try {
      ...
      /* storage blocked -- stays expanded, same as a real user's first visit */
    }
  }, [])
  ...
  const toggleCollapsed = () => {
    setCollapsed((prev) => {
      ...
      try {
        ...
        /* storage blocked -- toggle still works for this page view, just doesn't persist */
      }
      return next
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 lg:flex">
      ...
      <aside className={`hidden lg:flex lg:flex-col lg:fixed lg:top-8 lg:bottom-0 border-r border-gray-200 bg-white dark:bg-gray-900 dark:border-gray-800 transition-[width] duration-200 ${collapsed ? 'lg:w-16' : 'lg:w-64'}`}>
        <RailContents variant={variant} role={role} activeSection={activeSection} userName={userName} verified={verified} greyinScore={greyinScore} collapsed={collapsed} onOpenFeedback={() => setFeedbackOpen(true)} onLogNav={onLogNav} onLogPillarSwitch={onLogPillarSwitch} />
        <button
          type="button"
