...
export function FeedbackWishlistModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      // Opening the modal clears the unread-reply badge -- same "mark
      // read on the action that actually shows it" contract
      // NotificationBell's own item-click already uses, applied here to
      // the whole type at once since the badge itself is a type-scoped
      // count (SectionBadge), not a per-item list.
        ...
        <div className="flex items-center justify-between px-4 h-14 border-b border-gray-200 dark:border-gray-800 shrink-0">
          <div className="flex items-center gap-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium ${tab === 'feedback' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'}`}
            >
              Feedback
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium ${tab === 'wishlist' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400'}`}
            >
              Wishlist
            </button>
          </div>
            ...
            <>
              <form onSubmit={submitFeedback} className="space-y-3 mb-6">
                <div className="flex items-center gap-2 text-sm font-medium text-gray-900 dark:text-gray-50">
                  <MessageSquareHeart className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                  Share your feedback
                </div>
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 font-semibold text-sm disabled:opacity-50"
                >
                ...
                <div className="space-y-3">
                  ...
                  {myFeedback.map((f) => (
                    <div key={f.id} className="border border-gray-200 rounded-lg p-3 dark:border-gray-800">
                      ...
                      {f.admin_reply && (
                        <div className="mt-2 pl-3 border-l-2 border-indigo-200 text-sm text-gray-700 dark:border-indigo-900 dark:text-gray-300">
                          <p className="font-medium text-indigo-700 dark:text-indigo-400 text-xs mb-0.5">Team reply</p>
                          {f.admin_reply}
                        </div>
                      )}
                    </div>
                      ...
                      <div key={r.id} data-testid="wishlist-item" className="flex items-start gap-3 border border-gray-200 rounded-lg p-3 dark:border-gray-800">
                        <button
                          ...
                          className={`flex flex-col items-center gap-0.5 px-2.5 py-1.5 rounded-lg border text-xs font-semibold shrink-0 ${
                            upvoted ? 'bg-indigo-50 border-indigo-300 text-indigo-700 dark:bg-indigo-950/40 dark:border-indigo-800 dark:text-indigo-400' : 'border-gray-200 text-gray-600 dark:border-gray-800 dark:text-gray-400'
                          }`}
                        >
