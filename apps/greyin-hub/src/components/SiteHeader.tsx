    ...
    <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-50">
        ...
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex items-center flex-shrink-0">
            <span className="relative inline-block w-9 h-9 flex-shrink-0">
              ...
              <Image src="/logo.png" alt="" width={26} height={26} className="absolute inset-0 m-auto h-[26px] w-[26px]" priority />
            </span>
            <Image src="/GreyIn.png" alt="Greyin" width={130} height={21} className="ml-2 h-5 w-auto" priority />
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            {NAV_LINKS.map((link) => (
              <Link key={link.href} href={link.href} className="text-gray-700 hover:text-indigo-600 transition">
                {link.label}
              </Link>
            ))}
            {isLoggedIn === true ? (
      ...
      {mobileOpen && (
        <nav className="md:hidden border-t border-gray-200 bg-white px-4 py-4 space-y-1">
          <div className="flex items-center justify-around pb-3 mb-1 border-b border-gray-200">
            {PILLARS.map((p) => {
              const Icon = PILLAR_ICONS[p.key]
              return (
          ...
          {NAV_LINKS.map((link) => (
            <Link key={link.href} href={link.href} className="block py-2.5 text-gray-700 hover:text-indigo-600" onClick={() => setMobileOpen(false)}>
              {link.label}
            </Link>
          ))}
