  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Eligibility Threshold Governance</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Member vote results (advisory)</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-indigo-200 bg-indigo-50 rounded-lg p-4">
              <p className="flex items-center gap-1 text-sm font-semibold text-indigo-700 mb-2">
                <BadgeCheck className="h-4 w-4" /> Verified Experts
              </p>
              {verifiedBucket ? (
                <>
                  <p className="text-sm text-gray-700">{verifiedBucket.vote_count} votes</p>
                  <p className="text-sm text-gray-700">Avg proposed years: {verifiedBucket.avg_proposed_years} (median {verifiedBucket.median_proposed_years})</p>
                  <p className="text-sm text-gray-700">Avg proposed score: {verifiedBucket.avg_proposed_score} (median {verifiedBucket.median_proposed_score})</p>
                </>
              ) : (
            ...
            <div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
              <p className="flex items-center gap-1 text-sm font-semibold text-gray-700 mb-2">
                <Users className="h-4 w-4" /> Everyone else
              </p>
              {otherBucket ? (
                <>
                  <p className="text-sm text-gray-700">{otherBucket.vote_count} votes</p>
                  <p className="text-sm text-gray-700">Avg proposed years: {otherBucket.avg_proposed_years} (median {otherBucket.median_proposed_years})</p>
                  <p className="text-sm text-gray-700">Avg proposed score: {otherBucket.avg_proposed_score} (median {otherBucket.median_proposed_score})</p>
                </>
              ) : (
                <p className="text-sm text-gray-500">No votes yet.</p>
              )}
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Live gate settings</h2>
          ...
          <form action="/api/admin/gate-settings/update" method="POST" className="flex flex-col sm:flex-row gap-4 items-end">
            <div>
              <label htmlFor="min_years_experience" className="block text-sm font-medium text-gray-700">Min years experience</label>
              <input
                ...
                className="mt-1 block w-32 px-3 py-2 border border-gray-300 rounded-lg shadow-sm"
              />
            </div>
            <div>
              <label htmlFor="min_greyin_score" className="block text-sm font-medium text-gray-700">Min Greyin Score</label>
              <input
