    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="font-semibold text-gray-900 mb-4">Awaiting reply ({openFeedback?.length || 0})</h2>
          {openFeedback && openFeedback.length > 0 ? (
                ...
                <div key={f.id} className="py-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-900">{f.profiles?.full_name || f.profiles?.email}</span>
                      {f.source_app && <span className="text-xs text-gray-400 capitalize">{f.source_app}</span>}
                      {f.rating && (
            ...
            <p className="text-gray-500 text-sm">Nothing awaiting reply.</p>
          )}
        </div>

        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="font-semibold text-gray-900 mb-4">Recently replied</h2>
          {repliedFeedback && repliedFeedback.length > 0 ? (
            <div className="divide-y">
              {repliedFeedback.map((f: any) => (
                <div key={f.id} className="py-3">
                  <p className="text-sm font-medium text-gray-900">{f.profiles?.full_name || f.profiles?.email}</p>
                  <p className="text-sm text-gray-600 mb-1">{f.message}</p>
                  <p className="text-sm text-indigo-700 bg-indigo-50 rounded-lg px-3 py-1.5 inline-block">{f.admin_reply}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-gray-500 text-sm">Nothing replied to yet.</p>
          )}
        </div>
      </div>
    </main>
