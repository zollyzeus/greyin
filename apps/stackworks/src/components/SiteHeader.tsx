...
import { FlaskConical, Menu, X, ChevronDown, Home, MessageSquareHeart, Building2, BookOpen, Users, Briefcase, Telescope } from 'lucide-react'
...
/**
 * The audit's worst mobile finding: this header's nav text used to
 * overlap and clip the "StackWorks" wordmark on a phone screen, with the
 * "Sign Up" button cut off mid-word. Real collapse behavior instead of
 * an unbounded flex row fixes it.
 */
    ...
    <header className="bg-white dark:bg-gray-950 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        <a href="https://greyin.net" className="flex items-center min-w-0">
          <FlaskConical className="h-8 w-8 text-teal-600 dark:text-teal-500 flex-shrink-0" />
          <span className="ml-2 text-2xl font-bold text-gray-900 dark:text-gray-50 truncate">StackWorks</span>
        </a>

        <nav className="hidden md:flex items-center gap-6 flex-shrink-0">
          <a href="https://greyin.net" className="flex items-center gap-1 text-gray-700 dark:text-gray-300 hover:text-teal-600 dark:hover:text-teal-400" title="Back to the Greyin ecosystem hub">
            <Home className="h-4 w-4" />
            Home
          </a>
          <div className="relative group">
            <button className="text-gray-700 dark:text-gray-300 hover:text-teal-600 dark:hover:text-teal-400 transition flex items-center gap-1">
              Explore
            ...
            <div className="absolute left-0 mt-2 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all duration-200">
              {NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href} className="block px-4 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-teal-600 dark:hover:text-teal-400 transition">
                  {link.label}
                </Link>
              ))}
              <a href="https://greyin.net/feedback?app=stackworks" className="flex items-center gap-2 px-4 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-teal-600 dark:hover:text-teal-400 transition border-t border-gray-100 dark:border-gray-800" title="Send feedback to the Greyin team">
          ...
          {isLoggedIn === true ? (
            <>
              <Link href="/dashboard" className="text-gray-700 dark:text-gray-300 hover:text-teal-600 dark:hover:text-teal-400">Dashboard</Link>
              <form action="/auth/logout" method="POST">
                <button type="submit" className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 whitespace-nowrap">Sign Out</button>
              </form>
            </>
          ) : isLoggedIn === false ? (
            <>
              <Link href="/login" className="text-gray-700 dark:text-gray-300 hover:text-teal-600 dark:hover:text-teal-400">Sign In</Link>
              <Link href="/signup" className="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 whitespace-nowrap">
                Sign Up
              </Link>
            </>
          ) : null}
      ...
      {mobileOpen && (
        <nav className="md:hidden border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 px-4 py-4 space-y-1">
          <a href="https://greyin.net" className="flex items-center gap-2 py-2.5 text-gray-700 dark:text-gray-300 hover:text-teal-600 dark:hover:text-teal-400" onClick={() => setMobileOpen(false)}>
            <Home className="h-4 w-4" />
            Home
          </a>
          ...
          {exploreOpen && (
            <div className="pl-3 space-y-1 pb-1">
              {NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href} className="block py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-teal-600 dark:hover:text-teal-400" onClick={() => setMobileOpen(false)}>
                  {link.label}
                </Link>
              ))}
              <a href="https://greyin.net/feedback?app=stackworks" className="flex items-center gap-2 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-teal-600 dark:hover:text-teal-400" onClick={() => setMobileOpen(false)}>
                <MessageSquareHeart className="h-4 w-4" />
                Feedback
              </a>
            </div>
          ...
          <div className="pt-3 border-t border-gray-200 dark:border-gray-800 flex flex-col gap-2">
            {isLoggedIn === true ? (
              <>
                ...
                <form action="/auth/logout" method="POST">
                  <button type="submit" className="w-full text-center py-2.5 bg-teal-600 text-white rounded-lg">
                    Sign Out
                  </button>
                </form>
              </>
              ...
              <>
                ...
                <Link href="/signup" className="text-center py-2.5 bg-teal-600 text-white rounded-lg" onClick={() => setMobileOpen(false)}>
                  Sign Up
                </Link>
              </>
            ) : null}
