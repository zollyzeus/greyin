  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link href={`/projects/${id}`} className="flex items-center text-gray-600 hover:text-teal-600 mb-6">
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to {project.title}
        </Link>

        <div className="bg-white rounded-lg shadow-md p-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-6">Post an Ask</h1>

          <form action={`/api/projects/${id}/asks/create`} method="POST" className="space-y-6">
            <div>
              <label htmlFor="role_title" className="block text-sm font-medium text-gray-700">Role</label>
            ...
            <div>
              <label htmlFor="verification_criteria" className="block text-sm font-medium text-gray-700">
              ...
              <textarea id="verification_criteria" name="verification_criteria" rows={3}
                ...
                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-teal-500 focus:border-teal-500" />
              <p className="mt-1 text-xs text-gray-500">If set, AI verification checks submitted work against this specifically instead of just the description above.</p>
            </div>
