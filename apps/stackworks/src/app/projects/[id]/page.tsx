          ...
          {project.images && project.images.length > 0 && (
            <div className="flex flex-wrap gap-3 mb-6">
              {project.images.map((url: string) => (
                ...
                <img key={url} src={url} alt="" className="w-40 h-28 object-cover rounded-lg border border-gray-200" />
              ))}
            </div>
          )}
              ...
              <form action={`/api/projects/${id}/upvote`} method="POST">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm ${
                    hasUpvoted ? 'bg-teal-600 text-white' : 'bg-teal-50 text-teal-700 hover:bg-teal-100'
                  }`}
                >
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            {!shippedOutcome ? (
              <form action={`/api/projects/${id}/submit-outcome`} method="POST" className="space-y-3">
                <p className="text-sm text-gray-600">Submit this project for verification to earn a Verified badge on it.</p>
                <div>
                  <label htmlFor="ship-summary" className="block text-sm font-medium text-gray-700 mb-1">What did you ship?</label>
                  <textarea
                    id="ship-summary"
                    name="summary"
                    required
                ...
                <div>
                  <label htmlFor="ship-evidence" className="block text-sm font-medium text-gray-700 mb-1">Evidence link (optional)</label>
                  <input
                    id="ship-evidence"
                    name="evidence_url"
                    type="url"
              ...
              <div className="text-sm">
                <p className="text-gray-700 mb-2">Submitted — awaiting review.</p>
                {shippedOutcome.ai_score !== null ? (
                  <p className="text-gray-600">AI review: {shippedOutcome.ai_score}/100{shippedOutcome.ai_notes ? ` — ${shippedOutcome.ai_notes}` : ''}</p>
                ) : (
                  <p className="text-gray-500">AI review unavailable — an admin will review this manually.</p>
                )}
              </div>
            ) : shippedOutcome.status === 'rejected' ? (
              <p className="text-sm text-gray-600">This submission wasn&apos;t verified. Contact an admin if you think that&apos;s wrong.</p>
            ) : (
              <p className="text-sm text-gray-600">Verified — {shippedOutcome.score}/100.</p>
            )}
          </div>
        )}
              ...
              {asks.map((ask: any) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    <span className="font-semibold text-gray-900">{ask.role_title}</span>
                  ...
                  {ask.skills && ask.skills.length > 0 && (
                    <p className="text-sm text-gray-600">{ask.skills.join(', ')}</p>
                  )}
                </Link>
              ))}
            </div>
          ) : (
            <p className="text-gray-500 text-sm bg-white rounded-lg shadow p-4">No open asks yet.</p>
          )}
        </div>
            ...
            {updates?.map((update: any) => (
              <div key={update.id} className="bg-white rounded-lg shadow p-6">
                <div className="flex items-center gap-2 mb-2">
                  <span className="font-semibold text-gray-900">{update.profiles?.full_name || 'Builder'}</span>
                  <span className="text-xs text-gray-500">{new Date(update.created_at).toLocaleDateString()}</span>
                </div>
                <p className="text-gray-700 whitespace-pre-line">{update.body}</p>
              </div>
            ))}
            {(!updates || updates.length === 0) && (
              <p className="text-gray-500 text-sm bg-white rounded-lg shadow p-4">No updates yet.</p>
            )}
          </div>
        </div>
      </div>
