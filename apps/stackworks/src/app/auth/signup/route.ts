...
export async function POST(request: Request) {
  ...
  const track = formData.get('track') === 'builder' ? 'builder' : 'supporter'
  ...
  // platform_gate_settings vote actually takes effect here too. Read via a
  // plain stateless fetch, not the cookie-bound SSR client -- a brand-new
  // visitor hitting signup has no session yet, and making this the first
  // PostgREST call on that client here (before it's ever used for
  // auth.signUp()) was found to trigger a crash in @supabase/ssr's
  // session-recovery path under load. This is public config (anon-
  // readable), so no session is needed anyway.
  ...
  // Only the Builder track carries Salt & Pepper's years-of-experience
  // gate. An existing Salt & Pepper member never hits this route at all --
  // their years_experience already qualifies them via SSO on first login
  // (see auth/login/route.ts) -- this path is only for someone signing up
  // on stackworks directly for the first time.
  if (track === 'builder' && (!Number.isFinite(yearsExperience) || yearsExperience < minYearsExperience)) {
    return NextResponse.redirect(
      absoluteUrl(
        `/signup?track=builder&error=${encodeURIComponent(`Posting as a Builder requires ${minYearsExperience}+ years of experience. Join as a Supporter instead if that's not you yet.`)}`
      )
    )
  }
  ...
  const { error: authError } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        ...
        last_name: lastName,
        role: track === 'builder' ? 'member' : 'supporter',
        stackworks_role: track,
  ...
  if (authError) {
    return NextResponse.redirect(
      absoluteUrl(`/signup?track=${track}&error=${encodeURIComponent(authError.message)}`)
    )
  }

  // The profiles row is finalized by a DB trigger once the user verifies
  // their email (see migration 010, extended by 030 for stackworks_role) --
  // not here, so an abandoned/unverified signup never leaves a
