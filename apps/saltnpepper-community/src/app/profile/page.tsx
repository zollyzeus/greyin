...
export default async function ProfilePage() {
  ...
  // their own reads. reputation_scores is this app's own signal, but
  // wasn't shown anywhere on this page before either. Best-effort:
  // someone with no activity on a given pillar just shows nothing.
  ...
  const { data: reputationRow } = await supabase
    ...
    .eq('user_id', user.id)
    .maybeSingle()
  // Own AI-rated mentoring-reply scores (048_ai_quality_scores.sql) --
  // additive/informational only, does not feed greyin_score (unlike
  // GreyMatters' post score). Profile-only by design (user-confirmed) --
  // never shown inline in the live thread, so answering doesn't feel
  // like it's being publicly graded.
  const { data: replyQualityScores } = await supabase
  ...
  const verifiedAvg = verifiedCount > 0
    ...
    : null
  const replyQualityCount = replyQualityScores?.length || 0
  const replyQualityAvg = replyQualityCount > 0
    ? Math.round(replyQualityScores!.reduce((s, o) => s + (o.score || 0), 0) / replyQualityCount)
    : null
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {(verifiedCount > 0 || (profile?.total_reviews ?? 0) > 0 || (reputationRow?.score ?? 0) > 0 || replyQualityCount > 0) && (
          <div className="bg-white rounded-lg shadow p-6 mb-6 dark:bg-gray-900">
            ...
            {replyQualityCount > 0 && (
              <div className="mt-4 pt-4 border-t border-gray-100 text-sm dark:border-gray-800">
                <p className="text-gray-500 dark:text-gray-400">AI mentoring quality (this platform, informational only)</p>
                <p className="font-semibold text-gray-900 dark:text-gray-50">{replyQualityCount} scored repl{replyQualityCount === 1 ? 'y' : 'ies'} · avg {replyQualityAvg}/100</p>
              </div>
            )}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-gray-500 mt-1 dark:text-gray-400">This will be shown on your posts and comments</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100"
                />
                <p className="text-xs text-gray-500 mt-1 dark:text-gray-400">Separate interests with commas</p>
              </div>
            </div>
          </div>
