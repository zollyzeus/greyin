    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <a href="https://stackworks.greyin.net/admin/llm" className="text-sm font-medium text-gray-600 hover:text-purple-600">
            Manage AI providers →
          </a>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          ...
          {(flaggedDiscussions?.length || 0) + (flaggedReplies?.length || 0) > 0 ? (
              ...
              {flaggedDiscussions?.map((d) => (
                <div key={d.id} className="py-3 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <span className={`text-xs font-medium ${d.moderation_status === 'flagged_on_retry' ? 'text-red-600' : 'text-amber-600'}`}>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <form action="/api/admin/discussions/clear-flag" method="POST">
                      ...
                      <button type="submit" className="text-sm font-medium text-purple-600 hover:text-purple-700">Clear flag</button>
                    </form>
                    <form action="/api/admin/discussions/delete" method="POST">
                      <input type="hidden" name="discussion_id" value={d.id} />
                      <button type="submit" className="text-sm font-medium text-red-600 hover:text-red-700">Delete</button>
                    </form>
                  </div>
                </div>
              ))}
              {flaggedReplies?.map((r) => (
                <div key={r.id} className="py-3 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <span className={`text-xs font-medium ${r.moderation_status === 'flagged_on_retry' ? 'text-red-600' : 'text-amber-600'}`}>
            ...
            <p className="text-gray-500 text-sm">Nothing pending or flagged.</p>
          )}
        </div>
            ...
            {users?.map((u) => (
              <div key={u.id} className="py-3 flex items-center justify-between gap-4">
                ...
                <form action="/api/admin/users/update-role" method="POST" className="flex items-center gap-2">
                  ...
                  <button type="submit" className="text-sm font-medium text-purple-600 hover:text-purple-700">
                    Update
                  </button>
                </form>
              </div>
