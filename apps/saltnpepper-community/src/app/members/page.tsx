        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="flex gap-6">
              <Link href="/discussions" className="text-gray-700 hover:text-purple-600">Discussions</Link>
              <Link href="/members" className="text-purple-600 font-semibold">Members</Link>
            </nav>
          </div>
        </div>
      </header>
              ...
              {collaboratorProfiles.map((person) => (
                <div key={person.id} className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2">
                  ...
                  <a
                    ...
                    className="text-sm text-gray-800 hover:text-purple-700 hover:underline"
                  >
            ...
            {members.map((member) => (
              <div key={member.id} className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900">{member.full_name || 'Member'}</h3>
                    {member.location && <p className="text-sm text-gray-500">{member.location}</p>}
                  </div>
                </div>
                {!!scoreByUser.get(member.id) && (
                  <p className="flex items-center gap-1 text-xs font-medium text-amber-600 mb-2">
                    <Award className="h-3.5 w-3.5" />
                    {scoreByUser.get(member.id)} reputation
                  </p>
                )}
                {member.bio && <p className="text-sm text-gray-600 line-clamp-3 mb-2">{member.bio}</p>}
                {member.years_experience != null && (
                  <p className="text-xs text-purple-700 font-medium">{member.years_experience} years of experience</p>
                )}
